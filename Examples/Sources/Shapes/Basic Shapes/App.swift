...
@main struct BasicShapes: Applet {
	...
	func draw() {
		...
		OutlineRenderer2D.circle(at: circleX, 340, radius: 80, color: .darkBlue)
		...
		OutlineRenderer2D.rectangle(at: rectangleX - 40, 320, size: 80, 60, color: .orange)
		...
		OutlineRenderer2D.triangle(Vector2f(triangleX, 160), Vector2f(triangleX - 20, 230), Vector2f(triangleX + 20, 230), color: .darkBlue)
		
		// Polygon shapes and lines
		let polygonX = Window.width / 4 * 3
		Renderer2D.polygon(at: polygonX, 320, sides: 6, radius: 80, color: .brown)
		OutlineRenderer2D.polygon(at: polygonX, 320, sides: 6, radius: 80)
