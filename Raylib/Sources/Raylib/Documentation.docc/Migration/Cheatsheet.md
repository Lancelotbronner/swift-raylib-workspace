...
This document presents the eqquivalent APIs to help C programmers get started with the Swift bindings. This document is updated for **Raylib 4.0**.
...
Swift	|	``Window/enable(_:)``, ``WindowFlag/isEnabled``, ``WindowFlag/enable()``
...
Swift	|	``Window/disable(_:)``, ``WindowFlag/isEnabled``, ``WindowFlag/disable()``
...
Swift	|	``Window/place(at:_:)`` ``Window/position``
...
> Note: Only applicable if window is fullscreen (``Window/isFullscreen``)
...
> Note: Only applicable if window is resizeable (``WindowFlag/resizeable``)
...
Swift	|	``Window/width``, ``Window/size``
...
Swift	|	``Window/height``, ``Window/size``
...
Swift	|	``Monitor/count`` ``Monitor/all``
...
> Warning: Those functions are intended for advance users that want full control over the frame processing

> Note: By default `EndDrawing()` does this job: draws everything + `SwapScreenBuffer()` + manage frame timming + `PollInputEvents()`. To avoid that behaviour and control frame processes manually, enable in `config.h`: `SUPPORT_CUSTOM_FRAME_CONTROL`
